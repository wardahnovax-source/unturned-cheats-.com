...
const sourcePath =
	'C:/Users/Aman/.cursor/projects/c-Users-Aman-unturnedhacks/assets/c__Users_Aman_AppData_Roaming_Cursor_User_workspaceStorage_5fc01ad59007d6b1ec3564ba48b04eb7_images_image-4be7ee07-5934-4878-8eb6-1fb2f3143ab4.png';
...
async function generateSiteLogo() {
	...
	await writeFile(path.join(imagesDir, 'unturned-hacks-logo.png'), logoPng);
	console.log('Wrote public/images/unturned-hacks-logo.png (512×512)');

	const logoWebp = await sharp(logoPng).webp({ quality: 90, effort: 6 }).toBuffer();
	await writeFile(path.join(imagesDir, 'unturned-hacks-logo.webp'), logoWebp);
	console.log('Wrote public/images/unturned-hacks-logo.webp');
}
...
async function generateWebManifest() {
	const manifest = {
		name: 'Unturned Hacks',
		short_name: 'Unturned Hacks',
		description: 'Undetected unturned hacks — ESP, aimbot, radar and for PC',
		start_url: '/',
		display: 'standalone',
