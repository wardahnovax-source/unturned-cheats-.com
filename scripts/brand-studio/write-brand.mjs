...
export function validateBrandPayload(body) {
	...
	const logo = assetPath(b.logo ?? '/images/unturned-hacks-logo.webp');
	const logoRaster = assetPath(b.logoRaster ?? '/images/unturned-hacks-logo.png');
	const defaultOgImage = assetPath(b.defaultOgImage ?? '/images/unturned-hacks-combat.webp');
	const heroImage = assetPath(b.heroImage ?? '/images/unturned-hero-banner.webp');
...
function safezoneBlock(src, name) {
...
export function readBrandForStudio() {
	...
	const keywords = listMatch
		...
		: [];
	const seoBlock = safezoneBlock(src, 'seo');
	const copyBlock = safezoneBlock(src, 'copy');
	const sitemapBlock = safezoneBlock(src, 'sitemap');
	const themeBlock = safezoneBlock(src, 'theme');
	/** @type {Record<string, string>} */
	const seo = {};
